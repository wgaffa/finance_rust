...
use cqrs::{
    error::JournalError,
    events::{store::InMemoryStore, Event},
};
...
fn events() -> [Event; 2] {
    [
        ...
        Event::AccountOpened {
            ...
            name: Name::new("Groceries").unwrap(),
...
fn simple_journal_entry() {
    ...
    let expected = vec![
        Event::Journal {
            id: 1,
            description: String::from("Starting Balance"),
            date: Utc.ymd(2014, 5, 23),
        },
        Event::Transaction {
            account: Number::new(101).unwrap(),
            amount: Balance::credit(50).unwrap(),
            journal: 1,
        },
        Event::Transaction {
            account: Number::new(501).unwrap(),
            amount: Balance::debit(50).unwrap(),
            journal: 1,
        },
    ];
